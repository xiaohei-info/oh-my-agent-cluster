...
_ID_SEGMENT_RE = re.compile(r"^[A-Za-z0-9][A-Za-z0-9_-]*$")


def _validate_id_segment(value: str, field: str) -> None:
    if not _ID_SEGMENT_RE.fullmatch(value):
        raise ValueError(f"{field} must match [A-Za-z0-9][A-Za-z0-9_-]*")
...
def _load_action(raw) -> Action:
    ...
    _validate_id_segment(action_id, "action.id")
    step = raw.get("step")
...
def load_acceptance_doc(raw) -> AcceptanceDoc:
    ...
    for f in flows_raw:
        ...
        _validate_id_segment(flow_id, "flow.id")
