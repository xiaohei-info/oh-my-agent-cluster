...
## Dense Graph / DAG Visualization Rule

When designing an interactive DAG, Kanban graph, dependency graph, or workflow retrospective with many nodes, do **not** default to showing every node and every edge at once. A technically complete hairball is a failed design if the user cannot read dependencies.

Preferred pattern for dense task graphs:
- provide a **macro overview** first: stage/layer cards and main progression only
- separate **exception flows** into their own view, e.g. `blocked -> rework -> re-review -> QA/closeout` subway lanes
- provide **single-node focus**: choose one task and show only `parents -> self -> children`
- provide **swimlanes/grouped lists** by stage and role (`ROOT / IMPL / REVIEW / REWORK / QA / CLOSEOUT`) instead of overlapping boxes
- hide or de-emphasize all global edges by default; reveal neighborhoods on selection
- make dense labels searchable/filterable rather than shrinking text until illegible
- if the user complains that lines are unclear or boxes overlap, redesign the information architecture; do not just nudge SVG coordinates

For workflow retrospectives, the goal is not graph-theory purity. The goal is fast comprehension of execution chain, handoff gates, blocked/reopened paths, and accountability.
...
## Document-to-Interactive-HTML (Canvas Mode)

When the user wants to **convert an existing Markdown document** into a polished, interactive single-page HTML (à la Gemini NotebookLM Canvas), this is a distinct workflow from designing from scratch. See `references/document-to-interactive-html.md` for the external tool landscape and detailed patterns.

Key differences from default design flow:

- **Source material is fixed** — read the MD fully before designing; the content dictates the layout, not a brief
- **Classify doc type first**:
  - *Data-driven* (tables, metrics, charts) → Chartifact-style: Vega charts, sortable tables, reactive filters
  - *Narrative/knowledge* (concepts, sections, hierarchy) → Canvas-style: sidebar TOC, collapsible sections, search, reading progress
  - *Hybrid* → blend both
- **Must-have interactive patterns for knowledge docs**:
  - Sticky sidebar TOC with scroll-spy
  - Collapsible sections (`<details>/<summary>` or custom accordion)
  - In-page search/filter
  - Dark/light theme toggle
  - Key terms with tooltip definitions
  - Mermaid diagrams for relationships
  - Smooth scroll + reading progress indicator
- **No external tool fully automates this** — the most practical path is using this skill's process with document-specific patterns baked in

### Architecture-spec / design-doc variant

When the source document is a **software architecture / system design / technical plan** rather than a generic note, bias the canvas toward structured technical reading instead of marketing-like presentation.

Preferred patterns:
- treat major `#` sections as **collapsible architecture cards**
- keep `##` subsections visible inside each card for scanability
- add a compact **hero summary** with document title, section count, key decisions, and reading intent
- use a **sticky left navigation** with active-section highlighting for long specs
- include quick-jump actions such as **top**, **open questions**, or other decision-critical anchors
- preserve tables and code / pseudo-contract blocks as first-class visual elements
- if Mermaid rendering would require build/dependency overhead, prefer a **safe styled source block fallback** over breaking the single-file artifact
- default visual posture: dark, restrained, architecture-canvas aesthetic; avoid turning a serious design doc into a landing page

#### Interactive Architecture Visualization (Advanced)

When the user wants a **comprehensive interactive architecture overview** that combines multiple documents (PRD, tech specs, API contracts) into a single navigable HTML:

**Structure Pattern:**
- **Multi-view navigation**: Top nav bar switching between Overview, Features, Modules, Tech Stack, Flows
- **Interactive SVG diagram**: Clickable architecture nodes that open detail panels
- **Sidebar navigation**: Hierarchical menu organized by system layers
- **Detail panel**: Slide-in or overlay panel showing component details from parsed documents
- **Feature cards**: Grid of clickable cards representing functional modules

**Implementation approach:**
1. Parse all source documents first to extract:
   - System layers and components
   - Feature modules with descriptions
   - Technology mappings
   - Data flows and processes
2. Design a unified visual system:
   - Color-code by layer (frontend/backend/runtime/external)
   - Consistent card/panel styling
   - Smooth transitions between views
3. Build interactive elements:
   - SVG nodes with `data-detail` attributes
   - JavaScript state management for view switching
   - Detail panel content injection from parsed data
4. Ensure self-containment:
   - Embed all CSS and JavaScript inline
   - Use data URIs for any images
   - No external dependencies beyond fonts

**Example deliverable structure:**
```
AI-Team-Interactive-Architecture.html
├── Hero section (stats, description)
├── Interactive SVG architecture diagram (clickable nodes)
├── Detail panel (dynamic content injection)
├── Feature grid (12 core features with tags)
...
- JavaScript object to store detail data
- Event delegation for dynamic element handling
- CSS transitions for smooth panel animations

Verification guidance for this variant:
- first verify the file exists and is self-contained
- when possible, serve it locally over simple HTTP and check that the page responds successfully
- if full browser-stack visual verification is unavailable in the moment, explicitly report that limitation and still verify via file presence, HTTP response, and expected DOM/content markers
- never claim full visual/browser verification unless it actually happened
...
Use this mode when the user wants to **convert a structured Markdown document into an interactive, self-contained HTML page** — not design from scratch, but transform existing content into a more navigable, visually engaging format.

This is distinct from the default design workflow (brief → design system → artifact). Here the content already exists; the job is structure, navigation, and visual enhancement.

### When to Use

- User says "把这篇文档转成HTML页面" or "make this doc interactive"
- Input is a structured MD file with sections, tables, Q&A pairs, checklists
- Output target is a single self-contained HTML file with embedded CSS/JS
- Goal: make the content easier to navigate, scan, and understand than raw Markdown

### Workflow

1. **Read and parse** the source document. Identify structural patterns:
   - Numbered phases/stages → timeline navigation
   - Tables with role/type/question columns → collapsible Q&A cards
   - Bullet lists with checkboxes → interactive checklists
   - Key-value metadata (time/input/output) → meta badges
   - Narrative paragraphs → styled callout blocks
   - Design principles or tag lists → card grids / tag chips
2. **Choose a navigation model** based on document structure:
   - Sequential phases → top timeline with phase switching
   - Flat sections → sidebar TOC or accordion
   - Deep hierarchy → nested collapsible tree
3. **Map content to interactive components**:
   - Role tables → collapsible cards with role badge + stage label + Q&A items inside
   - Checklist items → clickable checkboxes with strike-through on completion
   - Principles → card grid with name + description
   - Tags/categories → colored chip rows
   - Deliverables → highlighted card with icon
4. **Design the visual system** per claude-design principles, but bias toward:
   - Dark theme (documents are often read for extended periods)
   - Phase-specific color coding if the document has stages
   - Subtle animations (fade-slide on section switch, rotate on card toggle)
   - Keyboard navigation (arrow keys for phase switching)
5. **Handle missing assets**: MD documents often reference images by relative/Obsidian paths. Options:
   - Replace with descriptive placeholder text
   - Note in the output that images need manual replacement
   - If images are accessible on disk, embed as base64 data URIs
6. **Verify**: check file completeness, HTML structure, no console errors if browser tools available.

### Key Patterns

- **Timeline nav**: horizontal dots with labels, CSS progress line, `--progress` variable for animation
- **Collapsible cards**: `.role-card` with `.open` class toggling `max-height` on body, click handler on header
- **Checklists**: `.checklist-item` with `.checked` class toggling visual state
- **Phase switching**: hide/show sections with `.active` class, CSS `fadeSlide` animation
- **Scroll-to-top**: fixed button with visibility based on `scrollY`

### Pitfalls

- Do NOT try to render Obsidian `![[image.pdf]]` or `![](../../relative/path.png)` references as-is — they won't resolve in a standalone HTML file. Replace with descriptive text or base64 embed.
- Do NOT assume the document's table structure is uniform — some rows span columns or have nested content. Parse carefully.
- Do NOT lose the original Q&A pairing when converting tables. Each question must stay linked to its answer/context.
- Do NOT over-design: the content is the star, not the UI. Navigation and visual hierarchy should serve comprehension, not decoration.
- Use `templates/document-canvas-phases.html` as a starter scaffold for phased documents (timeline nav + collapsible cards + checklists).
- See `references/document-canvas-ecosystem.md` for the GitHub ecosystem of Canvas/Artifact tools (Chartifact, Open Canvas, etc.) if the user asks about alternatives.
