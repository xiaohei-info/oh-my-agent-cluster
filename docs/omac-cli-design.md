...
- LLM 降级为被 CLI 派活的专家:planner(制定计划 + 验收文档)、orchestrator(拆 DAG + 增量扩展)、reviewer(评审)、worker(开发)、acceptor(总控验收)——每个都是**有终点的单次任务**,正是 LLM 擅长的形状;
...
```
   人 (终端)        Agent (Claude Code / Multica agent)      Web UI (未来)
     ...
     (计划+验收 (拆DAG+增量  (评审+评审 (开发+环境 (总控验收)
       文档)     扩展)        目标)      步骤)
```
...
| **planner** | 从零制定计划文档;计划定稿后产出**验收文档**(用户视角端到端验收点) | `plan create` | 计划文档 + 验收文档 |
| **orchestrator** | 把计划/设计文档拆解为 manifest DAG;总控验收发现问题后**增量扩展** DAG | `plan create` / 总控验收 fail 后 | manifest YAML(全量或增量节点) |
| **reviewer** | 评审计划文档、验收文档、manifest、代码 PR | plan 流水线内置门 / dag run 派发 | 结构化 verdict + report(**含评审目标**) |
...
- 全部角色都是 Multica squad 里的 agent,在 `omac init` 时完成映射。
...
omac
  CORE(调用者/驱动侧)
    plan     create | check | show         计划制定 + DAG 拆解流水线(内置双 review 门)
    dag      run | status | tick           确定性 loop 执行
    node     show | retry | abandon        exit 20 后的决策工具
  WORK(被派发 agent 侧)
    work     show | submit                 六种任务统一接口
  SETUP
    init     交互式配置 / --check 体检
    config   get | set
  GUIDE
...
| `5` | 校验失败(lint / 证据 schema) | plan check、work submit 等 |
...
roles:
  ...
  workers: [backend-agent, fe-agent]     # 显式列表,或——
  reviewers:
    from_role: reviewer                  # ——按 squad 成员平台角色自动发现
defaults:
  max_parallel: 4
  poll_interval: 30
  coverage_gate: 90
...
**参与者**:调用者、omac CLI、Multica 平台。目标:把 workspace / squad / 角色映射固化进 `.orchestrator/config.yaml`。
...
sequenceDiagram
    ...
    C-->>U: 请选择 workspace
    U->>C: 选定
    C->>M: multica squad list
    M-->>C: 小队列表
    C-->>U: 请选择 squad
    U->>C: 选定
    C->>M: multica squad member list <squad>
    M-->>C: 成员池(agent 名 + 平台角色)
    ...
    U->>C: omac init --check
    C->>M: 验证 CLI 已登录 / workspace·squad 存在 / 各角色 agent 在成员池
    C-->>U: 体检报告(缺什么、怎么补),全过则 exit 0
```

---

### 7.2 阶段二:`omac plan create` — 计划制定与 DAG 拆解流水线

**参与者**:调用者、omac CLI、Multica、planner、reviewer、orchestrator。

两种模式一条流水线:带 `--doc` 跳过 planner 制定计划环节;不带则 planner 先制定计划。计划定稿后(含评审通过),planner 再产出**验收文档**——先列出计划涉及的业务流程,再把每条流程转换为用户视角、端到端、可执行的验收动作(如 web 产品从前端页面走通全功能)。验收文档的双重作用:①需求目标的锚定点(orchestrator 拆解时 contract.acceptance 须锚定其条目,开发不跑偏);②总控验收阶段的验收目标清单(§7.6)。

**全部 review 门内部衔接,不暴露评审参数**,开关只有 `--no-review`(一刀切跳过所有 review 门)与 `--no-acceptance`(跳过验收文档环节)。每个 LLM 环节的修订循环有界(≤3 轮),耗尽则 exit 20 移交调用者。
...
sequenceDiagram
    ...
    alt 未提供 --doc(模式 B:从零制定计划)
        ...
        C->>M: 计划文档写入 issue metadata
        loop review 门 ①(≤3 轮修订)
            C->>M: 建 review issue(附计划文档)并 assign reviewer
            M->>R: AgentRuntime 拉起 reviewer
            R->>C: omac work submit <id> --verdict pass/reject --report-file r.yaml
            alt verdict = reject
                C->>M: 评审意见回贴 planning issue,重派 planner 修订
                M->>P: 拉起 planner 修订
                P->>C: 重新 submit 修订稿
            end
        end
    ...
    loop review 门(验收文档,≤3 轮修订)
        C->>M: 建 review issue(附验收文档)并 assign reviewer
        M->>R: AgentRuntime 拉起 reviewer
    ...
    loop review 门 ②(≤3 轮修订)
        C->>M: 建 review issue(附 manifest)并 assign reviewer
        M->>R: AgentRuntime 拉起 reviewer
        R->>C: omac work submit --verdict ...
        alt verdict = reject
            C->>M: 意见回贴,重派 orchestrator 修订
        end
    end
...
> `omac plan check <file>`:调用者自己拆好了 manifest 时,只走 lint 门 + review 门 ②。
> `omac plan show`:查看已注册 manifest 的摘要。

---

### 7.3 阶段三:`omac dag run` — 确定性 Loop 执行

**参与者**:调用者、omac CLI(loop 本体)、Multica、worker、reviewer。
...
sequenceDiagram
    ...
    loop 每轮 tick(内置 sleep,直到收敛或需决策)
        ...
        alt 配置了 CI 且节点已交 PR
            ...
            alt CI 失败
                C->>M: 失败摘要评论回 issue,节点回退 worker(有界 ≤3)
                M->>W: 拉起 worker 修复后重新 submit
            end
        end

        alt CI 通过(或未配置)且节点配有 reviewer
            C->>M: 节点标 in_review,建/派 review 任务给 reviewer
            M->>R: AgentRuntime 拉起 reviewer(issue body 含 bootstrap + worker 的环境构建步骤)
        ...
        alt reviewer pass 且配置了自动合并
            ...
            alt 冲突 / 合并失败
                C->>M: 冲突信息评论回 issue,节点回退 worker(有界 ≤3)
...
被派活的 agent 永远只需要两个命令。任务类型存于 issue metadata,`show` 按类型输出对应协议,`submit` 按类型校验交付物。任务类型共六种:

| 类型 | 承担者 | submit 交付参数 |
|---|---|---|
| `plan` | planner | `--plan-file` |
| `acceptance` | planner | `--acceptance-file`(业务流程 → 逐条验收动作) |
| `decompose` | orchestrator | `--manifest-file`(全量;增量扩展模式只含新增节点) |
| `develop` | worker | `--pr-url --verification-file`(env 依赖时须含 env_setup) |
| `review` | reviewer | `--verdict --report-file`(必含评审目标) |
| `final-acceptance` | acceptor | `--acceptance-results-file`(逐项 pass/fail + 问题记录) |
...
sequenceDiagram
    ...
    C-->>A: 任务类型 + 完整上下文 + 该类型的执行协议<br/>(worker:TDD·证据格式;reviewer:独立复跑·验收映射;…)

    A->>A: 执行(开发 / 评审 / 制定计划 / 拆 DAG)

    A->>C: omac work submit <issue-id> <按类型的交付参数>
    Note over A,C: worker: --pr-url + --verification-file(含 env_setup)<br/>reviewer: --verdict + --report-file(含评审目标)<br/>planner: --plan-file / --acceptance-file<br/>orchestrator: --manifest-file<br/>acceptor: --acceptance-results-file
...
**参与者**:调用者、omac CLI、(可选)Multica。核心原则:**重试是显式决策**,不再自动发生。
...
sequenceDiagram
    ...
    alt 决策:重试(可换人)
        ...
        U->>U: 直接编辑 manifest(或重跑 omac plan check 走门禁)
    end
...
**参与者**:omac CLI、Multica、acceptor、orchestrator。核心原则:**全部节点 done ≠ 可交付**。开发阶段收敛后,以 plan 阶段产出的验收文档为目标清单,做一次用户视角的端到端总控验收;发现的问题变成增量节点回到**同一个 DAG** 继续推进,不起新 DAG。
...
| `review_report.review_goals` | review 任务必填 | reviewer 附上**评审所依据的目标**(验收映射、覆盖率、集成门、设计引用);verdict 回写 worker issue 时目标与意见一并送达——开发者朝目标修,而不是只修列出的问题 |
...
| workspace/squad/成员池 | 数据面 | ✅ team | ✅ project | 角色映射已在 `config.yaml`,不依赖平台 |
...
- 角色系统在 `config.yaml`,不依赖平台 squad role;
...
|---|---|
| `list_members(scope)` | `multica squad member list <squad-id>`(未配 squad 退化 `multica agent list`) |
