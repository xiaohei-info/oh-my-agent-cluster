...
oh-my-multica 中高能力模型承担设计、规划和质量判断，
高性价比模型承担数量最多、Token 消耗最大的开发与测试节点。Loop Engineering 自动跟踪每个节点的依赖和状态，Harness
Engineering 提供行动前约束与行动后反馈；未通过 contract、验证、评审或最终验收的结果会进入返工，不能进入交付链。

Factory Missions 与 oh-my-multica 都能规划、并行开发和执行用户侧 QA，功能范围高度重叠。核心区别是
过程状态和恢复决策由谁掌握。Factory 的 Orchestrator 本身是 Agent，暂停、重新规划和恢复主要通过
继续与它对话完成，取决于模型的性能与Token的消耗，存在不确定性，而软件工程，特别是生产级的软件工程对于不确定性是厌恶的。oh-my-multica 则通过确定性的程序和流程保证交付事实，Agent 只执行有边界的设计、开发、
评审和验收任务，将确定性的程序与动态发挥的 Agent 有机结合起来，希望在其中找到最合适的边界点来让他们形成最大化的效率并转换为成果。
