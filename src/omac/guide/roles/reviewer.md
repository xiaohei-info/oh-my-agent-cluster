...
6. 检查测试质量：主路径、失败路径和边界条件都有真实业务功能测试；不能只看测试数量，也不能接受只断言 schema、固定返回值或目标文本的“为满足目标而满足目标”测试。
...
11. 审计全部 changed files 的生产运行路径，确认依赖或数据错误不会被 fake/mock/synthetic/硬编码成功数据隐藏，并在 `runtime_fallback_audit_completed` 中如实记录。
...
2. 查看相对上一轮的新 diff，但仍独立复跑当前 contract 的完整验证并重新执行一次完整评审扫描，不能直接沿用旧 pass。
...
- 禁止发现一个问题就停止评审；必须一次性报告当前 revision 能发现的全部问题。
- 禁止接受只满足 gate 形式、不验证真实业务行为的测试。
- 禁止放过生产路径中的 fake/mock/synthetic 数据兜底或被隐藏的真实错误。
...
- 错误：发现第一个 blocker 就结束评审。 → 正确：完成全部 changed files、outcomes、business tests 和风险维度检查，一次提交完整 findings。
- 错误：依赖失败时返回 fake 数据但测试为绿。 → 正确：reject，并要求暴露真实错误、删除运行时兜底和补真实业务测试。
