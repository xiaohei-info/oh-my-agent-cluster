...
真实案例：https://github.com/xiaohei-info/oh-my-multica/blob/main/docs/case-studies/building-v1-on-multica.zh-CN.md
...
5. **真实建设记录**：29/29 完成、168 次执行、15 次失败、8 次重试、26 个 PR 合并。
6. **失败为什么比精选 Demo 更有信息量**：超时、被替代的实现和接续执行。
7. **模型如何分层配置**：强模型做设计与判断，高性价比模型做大量有边界的执行任务。
8. **它不保证什么**：错误需求和无效测试不会因为有 Loop 就自动变正确。
9. **如何体验**：PyPI 安装、真实 Multica 前置条件与无需账号的 mock demo。

长文可以以中文案例
[`building-v1-on-multica.zh-CN.md`](../case-studies/building-v1-on-multica.zh-CN.md)
为事实主体，再补充 README 中 Loop Engineering 与 Harness Engineering 的两节。不要把 README 从头
到尾重新排版后当作文章。
